...
<section class="grid two planner-shell">
  ...
  <div class="card draft-panel">
    ...
    {{if .GenerationRunning}}<div class="callout"><strong>Generation running</strong><p>Planner/critic generation is running asynchronously. Refresh this page to inspect status; no task will be created until you approve the draft.</p></div>{{end}}
    ...
    <p class="hint">Internal planner/critic inputs and raw runtime logs are grouped by generation here only; they are not task artifacts or handoff content.</p>
...
<section class="actions card">
  ...
  {{if eq .Task.Status (index .K "TaskStatusNeedsFix")}}<div><strong>Fix requested</strong><p class="hint">Queue the next attempt with the current plan and review notes.</p></div><form method="post" action="/tasks/{{.Task.ID}}/resume-fix"><input type="hidden" name="csrf_token" value="{{.CSRFToken}}"><button type="submit">Queue fix attempt</button></form>{{end}}
...
<section class="task-tabs card" data-task-tabs>
  ...
  <section id="panel-activity" class="tab-section" role="tabpanel" aria-labelledby="tab-activity"><h2>Activity</h2><ul class="events">{{range .Events}}<li><strong>{{eventLabel .Type}}</strong>{{with eventDetail .}}<span>{{.}}</span>{{end}}</li>{{end}}</ul></section>
